...
#' @usage scale_fill_jerzeg(..., alpha = 1, begin = 0, end = 1, direction = 1, discrete = FALSE, option = "B")
...
scale_fill_jerzeg <- function (..., alpha = 1, begin = 0, end = 1, direction = 1, discrete = FALSE, option = "B") {
  if (discrete) {
    discrete_scale("fill", "jerzeg", jerz_pal(alpha, begin, end, direction, option), ...)
  }
  else {
    scale_fill_gradientn(colours = jerz(256, alpha, begin, end, direction, option), ...)
  }
}
